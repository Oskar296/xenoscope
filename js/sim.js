...
XS.genSpecimen=function(kingdomKey, tier){
  ...
  if(autotroph===null) autotroph = (kingdomKey==='Monera')? Math.random()<0.35 : Math.random()<0.45;

  // clone parts; add photosynthetic gear for alien autotrophs
  ...
  spec.food = spec.isVirus ? 'host' : (autotroph ? 'light' : 'glucose');

  // build drawable organelle instances + unique id list
  const inst=[]; const ids=[];
  const zByShape={bigvac:0,vac:1,wall:0,halo:0};
  for(const [id,n] of parts){
    if(!ids.includes(id)) ids.push(id);
    const single = ['membrane','wall_cellulose','wall_chitin','wall_pepti','capsule','vacuole_central','nucleoid','cytoskeleton','septa','plasmodesmata','envelope','capsid'].includes(id);
    for(let i=0;i<n;i++){
      inst.push(makeInst(id,i,n));
      if(single) break;
    }
  }
...
XS.freshState=function(spec, task){
  const S={ ph:spec.optPH, temp:spec.optT, light:0, glucose:0, minerals:0, host:0,
    antibiotic:0, antifungal:0, antiviral:0, lysozyme:0, hypotonic:0, hypertonic:0, toxin:0,
...
function threat(spec,S){
  ...
  const wrongAg=(S.antibiotic+S.antifungal+S.antiviral+S.lysozyme+S.hypotonic+S.hypertonic);
...
XS.simStep=function(spec,S,dt,tier){
  ...
  for(const k of ['light','glucose','minerals','host','antibiotic','antifungal','antiviral','lysozyme','hypotonic','hypertonic','toxin'])
...
function feed(s,S,amt){ if(s.isVirus)S.host=amt; else if(s.autotroph){S.light=amt;S.minerals=amt;} else S.glucose=amt; }
function foodHint(s){ return s.isVirus?'(Provide host cells.)':s.autotroph?'(Light + minerals.)':'(Glucose.)'; }
function killHint(s){
  const map={antibiotic:'Antibiotic works on bacteria.',antifungal:'Antifungal for fungi.',antiviral:'Antiviral for viruses.',
    hypotonic:'No wall → a hypotonic shock bursts it.',hypertonic:'Walled → a hypertonic shock plasmolyses it.',lysozyme:'Lysozyme cracks its wall.'};
  return map[s.kill[0]]||'';
}
